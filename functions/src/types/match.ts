...
export type MatchSession = {
  ...
  // Phase 9.4.5C: persisted only when mode === 'battle_mode'. Lets the
  // post-match UI ("Opponent's Deck" panel) reconstruct what the player
  // faced without needing to query player_saved_decks for the opponent
  // (which is on a different uid and would require admin-side reads).
  ...
  // Release 1.1.0 — quest tracking. Frozen at init time; counters
  // accumulate over the life of the match and are settled into
  // quest_progress at claim time. All optional / default 0 for lazy
  // migration of pre-1.1 matches.
