...
export const activateCommander = onCall<ActivateCommanderInput, Promise<ActivateCommanderResult>>(
  ...
  async (request) => {
    ...
    if (ctx.session[usedFlag]) {
      throw new HttpsError('failed-precondition', 'Commander already used this match.');
    }

    const commanderId = ctx.session[commanderIdField] as string;
    const cmdSnap = await db.collection('commander_library').doc(commanderId).get();
    if (!cmdSnap.exists) {
